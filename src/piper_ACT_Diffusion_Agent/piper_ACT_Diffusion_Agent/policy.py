...
class DiffusionPolicy(nn.Module):
    ...
    def __call__(self, qpos, image, actions=None, is_pad=None):
        ...
        if actions is not None: # training time
            ...
            #     nets = self.ema.copy_to(self.nets)
...
class ACTPolicy(nn.Module):
    ...
    def __call__(self, qpos, image,depth_image, actions=None, is_pad=None, vq_sample=None):
        env_state = None
        ...
        if actions is not None: # training time
            ...
            a_hat, [mu, logvar] = self.model(image=image, robot_state=qpos,actions=actions, action_is_pad=is_pad,
                                                                          depth_image=None)
            # a_hat, is_pad_hat, (mu, logvar), probs, binaries = self.model(image=image, robot_state=qpos,
            ...
            a_hat, [mu, logvar] = self.model(robot_state=qpos, image=image,depth_image=depth_image) # no action, sample from prior
            return a_hat
...
class CNNMLPPolicy(nn.Module):
    ...
    def __call__(self, qpos, image,depth_image=None, actions=None, is_pad=None):
        ...
        if actions is not None: # training time
            actions = actions[:, 0]
            a_hat = self.model(image=image,depth_image=None,robot_state=qpos,actions=actions)
            ...
            a_hat = self.model(robot_state=qpos, image=image, depth_image=depth_image) # no action, sample from prior
            return a_hat
