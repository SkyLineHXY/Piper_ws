...
class DiffusionTransformer(nn.Module):
    def __init__(self,obs_dim,action_dim,num_layers=6, num_heads=8, hidden_dim=512):
        super().__init__()


        # 输入映射 (从 action_dim -> hidden_dim)
        ...
        # 处理 global_cond 的 MLP
        self.global_encoder = nn.Linear(obs_dim, hidden_dim)
    ...
    def forward(self,sample,timestep:torch.Tensor,global_cond):
        """
            sample: (batch, T, action_dim) -> 加噪的 action
            timestep: (batch, 1) -> Diffusion timestep (用 embedding 处理)
            global_cond: (batch, obs_dim) -> 观测信息
        """
        B,T,_ =sample.shape

        # Step 1: 处理 global_cond，转换为 query (batch, 1, hidden_dim)
        global_query = self.global_encoder(global_cond).unsqueeze(1)
        timestep=timestep.float()
        # Step 2: 处理 timestep，转换为 embedding
        timestep_embed = self.time_embed(timestep).unsqueeze(0).unsqueeze(0)

        # sample_with_time = torch.cat([sample, timestep_embed], dim=-1)  # (batch, T, action_dim+1)

        # Step 3: 处理 sample（映射到 hidden_dim）
        sample_embedded = self.input_proj(sample)

        # Step 4: Transformer Encoder (Cross-Attention)
        transformer_input = torch.cat([global_query, timestep_embed, sample_embedded], dim=1)  # (batch, 2+T, hidden_dim)

        # Transformer Encoder
        encoded = self.transformer(transformer_input)  # (batch, T, obs_dim)

        encoded_seq = encoded[:, 2:, :]  # 只取 (batch, T, hidden_dim)
        # 取 Transformer 输出，通过 MLP 预测 noise
        noise_pred = self.output_mlp(encoded_seq)  # (batch, T, action_dim)
        return noise_pred
...
class Diffusion(nn.Module):
    def __init__(self, backbones, pools, linears, depth_backbones, state_dim, chunk_size,
                 ...
                 ema_power, camera_names,use_depth_image=False):
        """ Initializes the model.
        Parameters:
            backbones: torch module of the backbone to be used. See backbone.py
            transformer: torch module of the transformer architecture. See transformer.py
        ...
        self.noise_pred_net =DiffusionTransformer(
            obs_dim=self.obs_dim*self.observation_horizon,
        ...
        if ENABLE_EMA:
            '''
                ema 是 指数移动平均 (EMA) 版本的模型参数，用于训练平滑和稳定收敛。
                \theta_{ema}=\lambda \dot \theta_{ema-1} + (1-\lambda)\theta_{new}
            '''
