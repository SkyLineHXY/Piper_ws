...
class PiperRosNode(Node):
    def __init__(self) -> None:
        ...
        # Publishers
        self.joint_pub = self.create_publisher(JointState, 'slave_joint_states', 1)
        ...
        self.create_subscription(JointState, '/slave_piper_joint_ctrl', self.joint_callback, 1)
        ...
        while rclpy.ok():
            if(self.auto_enable):
                while not (enable_flag):
                    ...
                    self.get_logger().info(f"enable_flag is {enable_flag}")
        ...
        if (self.GetEnableFlag()):
            ...
            if not all_zeros:
                ...
                if lens == 7:
                    ...
                    self.piper.MotionCtrl_2(0x01, 0x01, 20)
            else:
                self.piper.MotionCtrl_2(0x01, 0x01, 20)
            ...
            if self.gripper_exist:
                if len(joint_data.effort) >= 7:
                    ...
                    if not math.isnan(gripper_effort):
                        ...
                        gripper_effort = 500  # You can set a default value in case of NaN
                    self.piper.GripperCtrl(abs(joint_6), gripper_effort, 0x01, 0)
                else:
                    self.piper.GripperCtrl(abs(joint_6), 500, 0x01, 0)
